          ...
          foreach($pendingTransactions->get() as $pendingTransaction){
            ...
            if ($repsonse['status'] == 1001){
                ...
                if(intval($values[2])>0){
                    EmailDispatcher::pendMails(parent::resolveMemberReference(trim($pendingTransaction->member_reference)), "YOU JUST EARNED LOYALTY POINTS ON THE FIRST REWARDS PROGRAMME", EmailDispatcher::buildTransactionTemplate(self::$placeholders, $values), 'no-reply@firstrewards.com');
                }
